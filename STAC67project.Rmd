...
#Model
Half the data set is choosen to create the model and other half is for validation.
Seed is set to 67 for consistent results.
```{r include = FALSE}
...
```
#Model Creation
Stepwise Regression with function stepAIC with 12 variables (Akaike's Information Criterion).
...
```
There are no observations with a Cooks Distance greater than the 20th percentile of F(10,496).
```{r include = FALSE}
DFBETAS <- dfbetas(fit)
head(DFBETAS)
which(DFBETAS > 1)
